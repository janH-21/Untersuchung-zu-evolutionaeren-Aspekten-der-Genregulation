...
// basal;
double basal;
double basal_iter[]{0, 0.1, 0.5, 1, 5, 10, 50}; //
...
max_program_counter = (sizeof(f_iter)/sizeof(f_iter[0])) * (sizeof(basal_iter)/sizeof(basal_iter[0]));

// basal loop
for(i=0; i<(sizeof(basal_iter)/sizeof(basal_iter[0])); i++) 
{
	...
	for(j=0; j<(sizeof(f_iter)/sizeof(f_iter[0])); j++)
	{
		...
		concat_file_name << fstart << "f-" << frequency << "_d0-" << d0 << "_k1-" << k1 << "_bas-" << basal;
		...
		while(t<t_max)
		{
			...
			// OEDs
			k0=sin(2*3.1415*frequency*t)+basal;
			mRNA+=(k0-mRNA*d0)*dt;
		...
		program_counter++;
		cout << "Finished " << program_counter << " of " << max_program_counter << " combinations; f = " << frequency << ", basal = " << basal << endl;
		//max_out.close();
